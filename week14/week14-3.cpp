///week14-3.cpp LeetCode 學習計畫 Linked list 第4題
///445. Add Two Numbers II
class Solution {
public:
    ListNode* myReverse(ListNode*head){  ///自訂函式(week14-2.cpp)
        if (head == nullptr || head->next == nullptr) return head; ///終止條件
        ListNode*ans=myReverse(head->next); ///函式呼叫函式
        head->next->next = head;///把前面，放到反過來後的後面
    ...
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
        ...
        ListNode*ans = myAddTwoNumbers(list1,list2); ///呼叫上週week13的程式
    ...
    ListNode*myAddTwoNumbers(ListNode*list1, ListNode*list2){
        ...
        if(carry>0)ans2->next = new ListNode(carry); ///進位處理
        return ans->next;
